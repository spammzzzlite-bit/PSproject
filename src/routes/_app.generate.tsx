...
import {
  Upload, FileText, Copy, Download, Save,
  Globe, Code2, CheckCircle2, ChevronDown, X,
  Check, Trash2, Edit2, ArrowDown, Plus, StopCircle,
  Coins, AlertTriangle, ChevronUp, ArrowRight
} from "lucide-react";
import {
  useProjects, useSuites, createTestCase, createSuite,
  useBugs, useRuns, useTestCases, deductTokens, createBug, createProject, addNotification, useTokens
} from "@/lib/store";
...
const MOCK_CASE_POOL = [
  { title: "Verify login with valid credentials", steps: "1. Navigate to login page\n2. Enter valid email\n3. Enter valid password\n4. Click Sign In", expected: "User is redirected to dashboard", priority: "high" as const },
  { title: "Verify login with invalid password", steps: "1. Navigate to login page\n2. Enter valid email\n3. Enter invalid password\n4. Click Sign In", expected: "Error message 'Invalid credentials' is shown", priority: "high" as const },
  { title: "Verify password reset flow", steps: "1. Click 'Forgot password'\n2. Enter registered email\n3. Click Send Reset Link\n4. Check email inbox", expected: "Reset email received with valid link", priority: "medium" as const },
  { title: "Verify session persistence on refresh", steps: "1. Login successfully\n2. Refresh the page", expected: "User remains logged in", priority: "medium" as const },
  { title: "Verify logout clears session", steps: "1. Login successfully\n2. Click Sign Out", expected: "User is redirected to login page, session is cleared", priority: "low" as const },
  { title: "Verify email field validation", steps: "1. Navigate to login page\n2. Leave email field empty\n3. Click Sign In", expected: "Validation error 'Email is required' is displayed", priority: "high" as const },
  { title: "Verify password minimum length", steps: "1. Navigate to signup page\n2. Enter password with < 8 characters\n3. Submit form", expected: "Error: 'Password must be at least 8 characters'", priority: "medium" as const },
  { title: "Verify Google OAuth redirect", steps: "1. Navigate to login page\n2. Click 'Sign in with Google'\n3. Complete Google auth flow", expected: "User is redirected back to dashboard with active session", priority: "high" as const },
  { title: "Verify rate limiting on login", steps: "1. Attempt login with wrong password 5 times\n2. Observe response after 5th attempt", expected: "Error: 'Too many attempts. Please wait.' is shown", priority: "medium" as const },
  { title: "Verify remember me checkbox", steps: "1. Navigate to login page\n2. Check 'Remember me'\n3. Login successfully\n4. Close browser and reopen", expected: "User remains logged in after browser restart", priority: "low" as const },
  { title: "Verify profile page loads user data", steps: "1. Login successfully\n2. Navigate to profile page", expected: "User name, email, and avatar are displayed correctly", priority: "medium" as const },
  { title: "Verify dark mode toggle", steps: "1. Login to dashboard\n2. Click theme toggle in settings\n3. Verify UI changes", expected: "All UI elements switch to dark mode palette", priority: "low" as const },
  { title: "Verify navigation breadcrumbs", steps: "1. Navigate to Projects > Project A > Suite B\n2. Observe breadcrumb trail", expected: "Breadcrumbs show 'Projects > Project A > Suite B' with clickable links", priority: "low" as const },
  { title: "Verify test case creation", steps: "1. Navigate to Test Suites\n2. Click '+ Add test case'\n3. Fill in title and steps\n4. Click Save", expected: "New test case appears in the suite list", priority: "high" as const },
  { title: "Verify test run execution", steps: "1. Navigate to a test suite\n2. Click 'Run All'\n3. Wait for execution to complete", expected: "Test run results are displayed with pass/fail status for each case", priority: "critical" as const },
];
...
function GeneratePage() {
  ...
  function generate() {
    if (tokens.plan !== "Premium" && tokens.balance < 15) {
      toast.error("Insufficient tokens to generate tests.");
      return;
    }
    ...
    // Stream cases one by one
    intervalRef.current = setInterval(() => {
      const idx = counterRef.current;
      if (idx >= MOCK_CASE_POOL.length) {
        // Natural finish
        if (intervalRef.current) clearInterval(intervalRef.current);
        intervalRef.current = null;
        setState("done");
        
        // Deduct tokens when naturally completed
        deductTokens(15);
        addNotification("Tokens Deducted", "Deducted 15 tokens for generating test cases.", "info");
        
        toast.success(`Generated ${MOCK_CASE_POOL.length} test cases`);
        return;
      }
      const c = MOCK_CASE_POOL[idx];
      const status = Math.random() > 0.2 ? "passed" : "failed";
      setGenerated((prev) => [...prev, { ...c, id: idx, status }]);
  ...
  function stopGenerating() {
    ...
    // Deduct tokens when stopped
    deductTokens(15);
    addNotification("Tokens Deducted", "Deducted 15 tokens for generating test cases.", "info");
    
    toast.success(`Generated ${counterRef.current} test cases`);
  }
  ...
  function handleSendToBugs(tc: GeneratedCase) {
    ...
    const stepsFormatted = stepsArray.map((step, idx) => `Step ${idx + 1}: ${step.trim()}`).join("\n");
...
${systemLogs}`;
  ...
  return (
    <div className="mx-auto max-w-5xl">
      <PageHeader section="§ Generate" title="Generate test cases" subtitle="Paste requirements, drop a file, enter a URL, or paste an API spec — then draft Playwright cases on demand." />

      <div className="space-y-6">
        {/* Feature Description (AI Prompt) */}
        <div className="rounded-[12px] border border-[var(--c-border)] bg-[var(--c-bg-card)] p-5 space-y-3">
          <label className="block font-mono text-[10px] font-medium uppercase tracking-[0.08em] text-[var(--c-text-muted)]">Feature Description (AI Prompt)</label>
          <textarea
          ...
          <div className="p-6">
            ...
            {inputTab === "file" && (
              <>
                ...
                >
                  {file ? (
                    <><FileText className="h-8 w-8 text-[var(--c-accent)]" /><p className="mt-3 font-medium text-[var(--c-text)]">{file.name}</p><p className="font-mono text-[11px] text-[var(--c-text-muted)]">{(file.size/1024).toFixed(1)} KB</p></>
                  ) : (
                    <><Upload className="h-8 w-8 text-[var(--c-text-muted)]" /><p className="mt-4 font-display text-[22px] text-[var(--c-text)]">Drop a file</p><p className="mt-1 text-[13px] text-[var(--c-text-muted)]">.pdf, .docx, .md, .txt</p></>
                  )}
                </div>
            ...
            {inputTab === "url" && (
              <div>
                <label className="mb-2 block font-mono text-[11px] font-medium uppercase tracking-[0.08em] text-[var(--c-text-muted)]">Page URL</label>
                <input
                  value={url}
                ...
                />
                <p className="mt-2 text-[12px] text-[var(--c-text-muted)]">We'll crawl the page and extract testable elements.</p>
              </div>
            )}
            {inputTab === "api" && (
              <textarea
        ...
        <div className="flex items-center justify-between">
          {state === "generating" && (
            <p className="font-mono text-[13px] text-[var(--c-text-muted)] animate-pulse">
              Generating… <span className="text-[var(--c-accent)] font-medium">({generated.length} test case{generated.length !== 1 ? "s" : ""} so far)</span>
            </p>
          )}
          ...
          {state === "generating" ? (
            <button
              ...
              className="inline-flex items-center gap-2 rounded-full bg-[var(--c-bg-hover)] px-[20px] py-[10px] text-[14px] font-medium text-[var(--c-text)] transition-all hover:bg-[var(--c-border-strong)]"
            >
        ...
        {(state === "done" || state === "generating") && visibleCases.length > 0 && (
          <div id="generate-results-container" className="grid gap-6 md:grid-cols-3 items-start relative">
            ...
            <div className="md:col-span-2 space-y-[20px]">
              <div className="flex items-center justify-between rounded-[12px] border border-[var(--c-border)] bg-[var(--c-bg-hover)] px-5 py-4">
                <p className="font-mono text-[10px] uppercase tracking-[0.12em] text-[var(--c-text-muted)]">Generated test cases</p>
                <span className="font-mono text-[11px] text-[var(--c-accent)] font-medium">{visibleCases.length} cases</span>
              </div>
              ...
              {state === "done" && (
                <div className="border-t border-[var(--c-border)] px-5 py-4 text-center bg-[var(--c-bg)]">
                  <span className="text-[13px] text-[var(--c-text-muted)]">
                    ...
                    <Link to="/suites" className="font-medium text-[var(--c-accent)] hover:underline">
                      Go to Test Suites →
                    </Link>
                  </span>
                </div>
        ...
        {activeScriptCase && (
          <div className="flex flex-col h-full space-y-4 text-white">
            <div className="flex items-center justify-between border-b border-[#2C2825] pb-4">
              <div>
                <p className="font-mono text-[10px] uppercase tracking-[0.12em] text-[#888]">PLAYWRIGHT SCRIPT</p>
  ...
  return (
    <div
      className={`rounded-[12px] border bg-[var(--c-bg-card)] p-5 shadow-[var(--shadow-sm)] transition-all duration-[var(--t-normal)] hover:-translate-y-[2px] hover:shadow-[var(--shadow-md)] ${
        tc.status === "passed" ? "border-l-[3px] border-l-[var(--c-pass)]" : "border-l-[3px] border-l-[var(--c-fail)]"
      }`}
    >
      {isEditing ? (
        /* Inline edit mode */
        <div className="space-y-4">
          <div>
            <label className="mb-2 block font-mono text-[10px] uppercase tracking-[0.08em] text-[var(--c-text-muted)]">Title</label>
            <input
          ...
          <div>
            <label className="mb-2 block font-mono text-[10px] uppercase tracking-[0.08em] text-[var(--c-text-muted)]">Steps</label>
            <textarea
          ...
          <div>
            <label className="mb-2 block font-mono text-[10px] uppercase tracking-[0.08em] text-[var(--c-text-muted)]">Expected Result</label>
            <input
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between gap-4 border-b border-[var(--c-border)] pb-3">
            <div className="flex items-center gap-2">
              ...
              <h4 className="text-[15px] font-bold text-[var(--c-text)] truncate max-w-[320px] md:max-w-[420px]">{tc.title}</h4>
            </div>
            
            <div className="flex items-center gap-2 shrink-0">
              {/* PASSED / FAILED badge */}
              <span className={`inline-flex items-center gap-1 rounded-sm px-2.5 py-0.5 font-mono text-[10px] font-bold uppercase tracking-wider text-white ${
                tc.status === "passed"
                  ? "bg-[var(--c-pass)]"
                  : "bg-[var(--c-fail)]"
              }`}>
              ...
              {/* Priority badge */}
              <span className={`inline-flex items-center rounded-sm px-2.5 py-0.5 font-mono text-[10px] font-semibold uppercase tracking-wider badge-${tc.priority}`}>
                {tc.priority}
              </span>
            </div>
          </div>
          ...
          <div className="grid gap-6 text-[13px] pt-3 md:grid-cols-2 relative">
            ...
            <div className="space-y-1.5 min-h-[80px]">
              <p className="font-mono text-[10px] font-bold uppercase tracking-[0.08em] text-[var(--c-text-muted)]">STEPS</p>
            ...
            <div className="space-y-1.5 min-h-[80px] md:pl-6">
              <p className="font-mono text-[10px] font-bold uppercase tracking-[0.08em] text-[var(--c-text-muted)]">EXPECTED RESULT</p>
          ...
          <div className="flex items-center justify-between gap-3 border-t border-[var(--c-border)] pt-4 mt-4">
            ...
            <div className="flex gap-2">
              ...
              <button
                onClick={() => {
                  if (!saveSuiteId) {
                    toast.error("Please select a project and suite first in the 'Save all to project' panel at the bottom right.");
                    return;
                  }
...
function SaveToProjectPanel({
  ...
  return (
    <div className="rounded-[12px] border border-[var(--c-border)] bg-[var(--c-bg-card)] p-5 space-y-4 shadow-[var(--shadow-sm)]">
      <p className="font-mono text-[10px] uppercase tracking-[0.12em] text-[var(--c-text-muted)]">SAVE ALL TO PROJECT</p>
      
      {/* Project Selector / Inline Creator */}
      <div className="space-y-1.5">
        <label className="block font-mono text-[9px] uppercase tracking-[0.08em] text-[var(--c-text-muted)]">Project</label>
        {isCreatingProj ? (
          <div className="flex gap-2">
            <input
              value={newProjName}
            ...
            <button onClick={() => setIsCreatingProj(false)} className="text-[12px] text-[var(--c-text-muted)]">Cancel</button>
          </div>
        ) : (
          <div className="flex gap-2 items-center">
            <select
              value={saveProjectId}
              onChange={(e) => { setSaveProjectId(e.target.value); setSaveSuiteId(""); }}
            ...
            <button onClick={() => setIsCreatingProj(true)} className="text-[12px] font-medium text-[var(--c-accent)] whitespace-nowrap hover:underline">+ New Project</button>
          </div>
        )}
      </div>
      ...
      {saveProjectId && (
        <div className="space-y-1.5">
          <label className="block font-mono text-[9px] uppercase tracking-[0.08em] text-[var(--c-text-muted)]">Suite</label>
          {isCreatingSuite ? (
            <div className="flex gap-2">
              <input
              ...
              <button onClick={() => setIsCreatingSuite(false)} className="text-[12px] text-[var(--c-text-muted)]">Cancel</button>
            </div>
          ) : (
            <div className="flex gap-2 items-center">
              <select
              ...
              >
                ...
                {projectSuites.map((s) => <option key={s.id} value={s.id}>{s.name}</option>)}
              </select>
              <button onClick={() => setIsCreatingSuite(true)} className="text-[12px] font-medium text-[var(--c-accent)] whitespace-nowrap hover:underline">+ New Suite</button>
            </div>
          )}
        </div>
      )}
